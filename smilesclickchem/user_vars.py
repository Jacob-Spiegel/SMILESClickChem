...
def save_vars_as_json(vars):
    ...
    It saves the file to the output_directory + "vars.json"
        -If SMILESClickChem has been run multiple times for the same directory it
...
def determine_bash_timeout_vs_gtimeout():
    ...
    try:  # timeout or gtimeout
        ...
        raise Exception(
            ...
            by SMILESClickChem or you may need to execute through Bash."
        )
    if timeout_result == 0:
    ...
    try:  # timeout or gtimeout
        ...
        raise Exception(
            ...
            SMILESClickChem or you may need to execute through Bash."
        )
                ...
                if type(argv[key]) == int or type(argv[key]) == float:
                    ...
                    try:
                        ...
                        if type(temp_item) == float:
                            ...
                            printout = (
                                printout
                                + "Please check SMILESClickChem documentation using -h"
                            )
                        ...
                        printout = (
                            printout + "Please check SMILESClickChem documentation using -h"
                        )
                        raise IOError(printout)
                else:
                    printout = "This parameter is the wrong type. \n \t Check :"
                    ...
                    printout = printout + "Please check SMILESClickChem documentation using -h"
                    raise IOError(printout)
...
def load_in_commandline_parameters(argv):
    ...
    printout = "(RE)STARTING SMILESClickChem 1.0: " + str(datetime.datetime.now())
    ...
    if vars["alternative_filter"] is not None:
        ...
        if len(scripts_to_copy) != 0:
            for filter_info in scripts_to_copy:
                ...
                print(
                    "SMILESClickChem will need to be restarted once all custom scripts \
            ...
            print("SMILESClickChem has incorporated the custom files into"
                  + " the filter Module.")
            print(
                " SMILESClickChem needs to be restarted and should now "
