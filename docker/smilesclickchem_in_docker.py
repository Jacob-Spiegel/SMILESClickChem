#!/usr/bin/env python
"""
This script will handle running SMILESClickChem in a docker container.
It handles generating the docker image and container, handling the user variables,
executing SMILESClickChem, and copying the files from the container to the desired directory.

This script requires a JSON file that contains all the parameters that would be
required to run SMILESClickChem on a host system (ie paths on your computer).
Necessary files, such as the source compound file, will be copied into the docker.

To run SMILESClickChem from within docker. Launches docker
...
The resulting SMILESClickChem output to the desired root_output_folder.

An example JSON is provided in: ./sample_SMILESClickChem_docker_json.json


To run SMILESClickChem in a docker, please run the `smilesclickchem_in_docker.py` script:
    Example on Linux/MacOS:
        #  cd to this directory in a bash terminal
        1) cd SMILESClickChem/docker/
        # Run smilesclickchem_in_docker.py with sudo and supply a json file using the
        # normal pathing of your system.
        2) `sudo python smilesclickchem_in_docker.py -j ./examples/sample_SMILESClickChem_docker_json.json`
    ...
    Example on Windows OS:
        ...
        #  cd to this directory in a bash terminal
        3) cd SMILESClickChem/docker/
        4)  `python smilesclickchem_in_docker.py -j ./examples/sample_SMILESClickChem_docker_json.json`
...
def adjust_dockerfile():
    ...
    to the windows version (run_smilesclickchem_in_container.bash) if not it will
    ...
    To:
        # ENTRYPOINT ["bash", "/smilesclickchem/run_smilesclickchem_in_container.bash"]
        ENTRYPOINT ["bash", "/smilesclickchem/run_smilesclickchem_in_container_windows.bash"]
    """
    printout = ""
    normal_entry = "/smilesclickchem/run_smilesclickchem_in_container.bash"
    windows_entry = "/smilesclickchem/run_smilesclickchem_in_container_windows.bash"
    replacement_line = (
        'ENTRYPOINT ["bash", "/smilesclickchem/run_smilesclickchem_in_container_windows.bash"]\n'
    )
...
def make_docker():
    """
    This will create the docker to run SMILESClickChem.
    ...
    print("Creating new docker image for SMILESClickChem")
    ...
    print(printout)
    try:
        os.system("docker build -t smilesclickchem . > {}".format(log_file))
    except:
        printout = (
            "\nCan not create a docker file. Please make sure to run the "
            + "script with sudo/administrative privileges.\nIf Linux/MacOS:\n"
            + "\t'sudo python smilesclickchem_in_docker.py -j PATH/JSON.json'\n"
            + "If Windows:\n\tRun from bash and "
    ...
    # Remove the temporary SMILESClickChem directory
    shutil.rmtree("SMILESClickChem")
...
def check_for_required_inputs(json_vars):
    ...
    if len(missing_variables) != 0:
        printout = "\nRequired variables are missing from the input. A description \
            of each of these can be found by running python ./RunSMILESClickChem -h"
...
def move_files_to_temp_dir(json_vars):
    ...
    # Copy over SMILESClickChem files into a temp directory
    temp_smilesclickchem_path = os.path.abspath("SMILESClickChem") + os.sep

    script_dir = str(os.path.dirname(os.path.realpath(__file__)))
    smilesclickchem_top_dir = str(os.path.dirname(script_dir))
    if os.path.exists(temp_smilesclickchem_path):
        shutil.rmtree(temp_smilesclickchem_path)
    os.mkdir(temp_smilesclickchem_path)
    smilesclickchem_top_dir = smilesclickchem_top_dir + os.sep
    change_permissions_recursively(temp_smilesclickchem_path)

    # Copy all files in SMILESClickChem directory into a temp except the Docker folder
    for fol_to_copy in [
        "smilesclickchem",
        "source_compounds",
        "accessory_scripts",
        ...
        shutil.copytree(
            smilesclickchem_top_dir + fol_to_copy, temp_smilesclickchem_path + fol_to_copy
        )
    shutil.copyfile(
        smilesclickchem_top_dir + "RunSMILESClickChem.py", temp_smilesclickchem_path + "RunSMILESClickChem.py"
    )
    # Open permissions
    change_permissions_recursively(temp_smilesclickchem_path)
...
def run_SMILESClickChem_docker_main(vars):
    """
    This function runs the processing to:
        ...
        5) execute RunSMILESClickChem.py from within the docker container
    ...
    """
    printout = (
        "\n\nThis script builds a docker for SMILESClickChem and runs SMILESClickChem "
        + "within the docker. The setup may take a few minutes the first time being run "
        + "and SMILESClickChem may take a long time depending on the settings.\n\n"
    )
    print(printout)
    ...
    # Run part 5) run SMILESClickChem in the container
    print("\nRunning SMILESClickChem in Docker")

    command = "docker run --rm -it -v {}:/Outputfolder/".format(outfolder_path)
    command = command + " smilesclickchem  --name smilesclickchem  --{}".format(run_num)
    # Execute SMILESClickChem
    print(command)
    os.system(command)
    change_permissions_recursively(outfolder_path)
    print("SMILESClickChem Results placed in: {}".format(outfolder_path))
...
run_SMILESClickChem_docker_main(ARGS_DICT)
